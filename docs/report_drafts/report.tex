...
\begin{abstract}
感应淬火作为一种关键的表面热处理工艺，其过程涉及电磁场、温度场与微观组织相变的高度非线性耦合，传统有限元仿真（FEM）方法计算耗时巨大，难以满足工艺参数快速优化的需求。本项目旨在开发一种基于深度学习的代理模型，以实现对轴类工件感应淬火过程的实时、高精度预测。
...
\section{引言}
本项目是机器学习课程的项目，通过机器学习预测轴类感应淬火的温度场和相变场。
...
\begin{figure}[htbp]
    \centering
    % \includegraphics[width=0.9\textwidth]{figure/parallel_u_fno_arch.png} % 待绘制
    \fbox{\parbox[c][5cm]{0.9\textwidth}{\centering Parallel U-FNO 整体架构图 (待绘制)}}
    \caption{Parallel U-FNO 整体架构图：展示了 FNO 分支与 U-Net 分支的并行结构以及最终的融合模块。}
...
\begin{enumerate}
    ...
    \begin{figure}[htbp]
        \centering
        % \includegraphics[width=0.7\textwidth]{figure/fusion_module.png} % 待绘制
...
\begin{itemize}
    \item \textbf{架构级保证}：我们在模型的相变输出头末端直接使用了 Softmax 激活函数。
    \item \textbf{优势}：这从数学结构上天然保证了输出的相变概率分布归一化，无需在损失函数中添加额外的物理惩罚项（如 $\gamma \cdot |1 - \sum P|$），从而简化了优化目标，提高了训练稳定性。
\end{itemize}
...
\begin{table}[htbp]
    ...
    \begin{tabular}{lcc}
        ...
        整体  & $\mathbf{3.73 \times 10^{-5}}$ & $\mathbf{0.0030}$ \\
        \hline
    \end{tabular}
\end{table}
