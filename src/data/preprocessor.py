...
# ================= 配置区域 =================
INPUT_FILE = "data/processed/processed_dataset.csv"  # 清洗后的长表文件
OUTPUT_DIR = "data/training_data"  # 输出文件夹
GRID_SIZE = (64, 128)  # (Width/r, Height/z)
NORM_FILE = "normalization_stats.json"  # 保存归一化参数的文件
MASK_FILE = "geometry_mask.npy"  # 保存几何形状掩码的文件
# ===========================================
...
def process_and_save_tensors():
    ...
    with open(os.path.join(OUTPUT_DIR, NORM_FILE), "w") as f:
        json.dump(stats, f)
    ...
    for i, ((freq_val, curr_val), group_df) in enumerate(groups):
        ...
        if not mask_saved:
            ...
            # 保存掩码
            np.save(os.path.join(OUTPUT_DIR, MASK_FILE), single_step_mask)
        ...
        np.save(os.path.join(OUTPUT_DIR, filename), tensor_final)
    ...
    print("\n\n全部完成！")
    print(f"1. 训练数据 (.npy) 保存在: {OUTPUT_DIR}/")
    print(f"2. 几何掩码 (Mask) 保存在: {OUTPUT_DIR}/{MASK_FILE}")
    print("   (训练时请加载此 Mask，计算 Loss 时只计算 mask=1 的区域)")
    print(f"3. 归一化参数保存在: {OUTPUT_DIR}/{NORM_FILE}")
    print("   (预测推理时，用这些参数把 0-1 的温度还原回摄氏度)")


if __name__ == "__main__":
    process_and_save_tensors()
